...
def get_input():
    while True:
        try:
            ...
            # エラー処理　折り紙構造体の必要十分条件を満たしているかどうか。
            if (d - a / math.cos(math.radians(c))) < 0:
                print(f"エラー：{d - a / math.cos(math.radians(c))}です。もう一度入力してください。")
                continue
            
            # エラー処理bの値は、0以上180以下で入力する必要がある。
            if b > 180 or b < 0:
                print("なす角φ[°]の値は0以上180以下で入力してください。")
                continue

            # エラー処理cの値は、0より大きく90未満である必要がある。
            if c >= 90 or c <= 0:
...
for j in range(0, f):
    for i in range(1, 10):
        if i == 1:
            if j == 0 and flag7 == 0:
                flag7 = 1
                faces.append([1, 2, 10, 4, 13])
            
            else:
                faces.append([1 + 17 * j, 2 + 17 * j, 10 + 17 * j, 4 + 17 * j, 14 + 17 * (j - 1)])
    
        elif i == 2:
            faces.append([5 + 17 * j, 6 + 17 * j, 10 + 17 * j, 4 + 17 * j, 14 + 17 * j])

        elif i == 3:
            faces.append([2 + 17 * j, 10 + 17 * j, 15 + 17 * j, 12 + 17 * j])

        elif i == 4:
            faces.append([6 + 17 * j, 10 + 17 * j, 15 + 7 * j, 8 + 17 * j])

        elif i == 5:
           if j == 0:
               faces.append([16, 12, 2])

        elif i == 6:
           if j == 0:
               faces.append([2, 16, 13])
           ...
           if f == 1 and flag1 == 0:
               ...
               flag2 = 1
               faces.append([17 * f, 8 + 17 * (f - 1), 6 + 17 * (f - 1)])
        
        elif i == 8:
            if f == 1 and flag3 == 0:
                flag3 = 1
                faces.append([6, 17, 14])
            
            elif flag4 == 0:
                flag4 = 1
                faces.append([6 + 17 * (f - 1), 17 * f, 14 + 17 * (f - 1)])

        elif i == 9:
            if j == 0:
                faces.append([4, 13, 16, 15])
                faces.append([4 + 17 * (f - 1), 14 + 17 * (f - 1), 17 + 17 * (f - 1), 15 + 17 * (f - 1)])
            
            else:
                faces.append([4 + 17 *(j - 1), 14 + 17 * (j - 1), 21 + 17*(j - 1), 32 + 17 * (j - 1), 8 + 17 * (j - 1), 15 + 17 * (j - 1)])
...
# グラフを画像として保存する。
plt.savefig("origami_structure.png", dpi=300, bbox_inches='tight')
